...
/**     @module/class substation.Authentication
    @root
    When clustering, prepares the cluster port for authentication requests to/from other processes.
...
/**     @local/class FauxCookies
    @development
    Simulates a [cookies]() instance, building a little serializable command package that can be
    applied to a real [cookies]() instance in another process.

    To ship this across processes, access the `commands` property manually.
@member/Array commands
    An Array of command info Arrays, for example `[ 'set', [ name, value, options ] ]`
*/
function FauxCookies (cookies) {
    this.cookies = cookies;
    this.commands = [];
}


/**     @member/Function FauxCookies#get

*/
FauxCookies.prototype.get = function (name) {
    if (!Object.hasOwnProperty.call (this.cookies, name))
        return;
    return this.cookies[name];
};


/**     @member/Function FauxCookies#set

*/
FauxCookies.prototype.set = function (name, value, options) {
    if (value === undefined) {
        delete this.cookies[name];
        this.commands.push ([ 'set', [ name ] ]);
    } else {
        this.cookies[name] = value;
        this.commands.push ([ 'set', [ name, value, options ] ]);
    }
};


/**     @property/Function FauxCookies.deserialize
    Apply a (commands Array)[@FauxCookies#commands) to a real (cookies)[] instance.
*/
FauxCookies.deserialize = function (package, cookies) {
    for (var i=0,j=package.length; i<j; i++) {
        var job = package[i];
        cookies[job[0]].apply (cookies, job[1]);
    }
};
...
@callback
*/
Authentication.prototype.setActive = function (user, client, rememberMe, cookies, callback) {
...
@callback
*/
Authentication.prototype.setIdle = function (user, client, rememberMe, cookies, callback) {
    var config = this.config;
    var sessionCache = this.sessionCache;
...
Authentication.prototype.logout = function (user, client, cookies, callback) {
    ...
    SessionsCollection.findAndModify (
        ...
        function (err, rec) {
            ...
            var now = (new Date()).getTime();
            if (rec.v && now - rec.c < config.sessionLifespan)
                backplane.broadcast ('session', 'logout', user, client);
...
*/
Authentication.prototype.getSession = function (path, cookies, request, response, callback) {
