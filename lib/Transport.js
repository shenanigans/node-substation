...
Transport.prototype.createReactions = function(){
    ...
    */
    this.reaction_REST = function (request, response) {
        ...
        router.getAction (request.url, request.method, function (action, params) {
            if (!action) {
    ...
    */
    this.reaction_SocketIO = function (socket, callback) {
        ...
        var initReply = new Reply (function (status, events, content) {
            station.logger.info ({ status:status, events:events });
            if (!events) return;
            socket.emit ('reply', { status:200, events:events });
        });
        station.emit ('liveConnection', socket.agent, initReply);
