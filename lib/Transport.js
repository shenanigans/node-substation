...
@member/Number port
    @default `80`
    Accept `http` and `Socket.io` connection on this port.
@member/Boolean allowLiveGuests
    @default `false`
    Whether to allow unregistered users to open a `Socket.io` connection. Guest users are not
    addressable and cannot receive events or peer connections. When `false`, an active live user who
    logged in without the "remember me" flag and then logs out will be kicked off their active
    connections.
@member/Boolean allowLiveIdlers
    @default `false`
    Whether to allow users with expired session tokens to open a `Socket.io` connection. Idle users
    will not receive events or peer connections. When `false`, active live users will be kicked off
    the line when set idle or logged out.
    ...
    */
    this.handshake_SocketIO = function (socket, callback) {
        ...
        authentication.getSession (path, cookies, function (err, agent) {
            ...
            if (!allowGuest && (!agent.user || !agent.client))
