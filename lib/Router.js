...
/**     @submodule substation:Configuration
*/
var DEFAULT_CONFIG = {
...
Router.prototype.addAction = function (/* method, route, action */) {
    ...
    if (!action)
        throw new Error ('blank action submitted');
...
Router.prototype.getOptions = function (request, callback) {
    ...
    for (var i=0,j=this.actions.length; i<j; i++) {
        ...
        if (action.config.hideSchema || ( !action.querySchemaExport && !action.bodySchemaExport ))
            methods[method] = true;
        else {
